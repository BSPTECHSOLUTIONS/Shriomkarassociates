  ...
  return (
    <div className="flex flex-col min-h-screen pt-24 md:pt-[130px]">
      ...
      <section className="relative h-[35vh] min-h-[280px] flex items-center bg-secondary overflow-hidden">
        <img
          src="/assets/img/projects/aloor-home/Aloor 5.jpg"
          alt="Our Projects"
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="max-w-3xl">
            ...
            <motion.h1 variants={fadeInUp} className="font-serif text-4xl md:text-6xl font-bold leading-tight mb-2 text-white">
              Building Landmarks.
            </motion.h1>
            <motion.h1 variants={fadeInUp} className="font-serif text-4xl md:text-6xl font-bold leading-tight mb-4 text-primary">
              Creating Legacies.
            </motion.h1>
            <motion.p variants={fadeInUp} className="text-lg text-white/80 font-sans max-w-xl">
              Explore our diverse portfolio across residential, commercial, and industrial constructions.
            </motion.p>
          </motion.div>
        </div>
      </section>

      {/* FEATURED PROJECTS — 6 large cards */}
      <section className="py-16 bg-muted/30">
        <div className="container mx-auto px-4 md:px-8">
          ...
          <h2 className="font-serif text-3xl md:text-4xl font-bold text-secondary mb-10">
            Our Landmark Builds
          </h2>
          ...
          >
            {featuredProjects.map((project) => (
              <motion.div
                key={project.id}
                variants={fadeInUp}
                className="bg-white rounded-2xl shadow-lg hover:shadow-2xl overflow-hidden border border-border/40 group flex flex-col cursor-pointer transition-all duration-300 hover:-translate-y-1"
                onClick={() => { setSelectedProject(project); setActiveTab("images"); }}
              >
                <div className="relative h-[260px] overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 flex gap-2 flex-wrap">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded shadow-sm ${getBadgeColor(project.category, project.status)}`}>
                      {project.status === "ongoing" ? "Ongoing" : project.category}
                    </span>
                    <span className="text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded shadow-sm bg-primary/90 text-secondary">
                      ★ Featured
                    </span>
                  </div>
                  {project.videos && project.videos.length > 0 && (
                    <div className="absolute top-4 right-4">
                      <span className="bg-black/70 text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1">
                        <Play size={10} className="fill-white" /> {project.videos.length} Videos
                      </span>
                    </div>
                  )}
                  <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="bg-white/90 text-secondary text-xs font-bold px-3 py-1.5 rounded-full">
                      VIEW DETAILS →
                    </span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`px-5 py-2 rounded-full text-xs font-bold tracking-wider transition-all ${
                    activeFilter === filter
                      ...
                      : "bg-muted text-secondary hover:bg-primary/20 hover:text-secondary"
                  }`}
                >
                  {filter}
                </button>
            ...
            <AnimatePresence>
              {filteredProjects.map((project) => (
                <motion.div
                  ...
                  className="bg-white rounded-xl shadow-md hover:shadow-xl overflow-hidden border border-border/40 group flex flex-col cursor-pointer transition-shadow"
                  onClick={() => { setSelectedProject(project); setActiveTab("images"); }}
                >
                  <div className="relative h-[200px] overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex gap-1.5 flex-wrap">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded shadow-sm ${getBadgeColor(project.category, project.status)}`}>
                        {project.status === "ongoing" ? "Ongoing" : project.category}
                      </span>
                      {project.featured && (
                        <span className="text-[10px] font-bold uppercase px-2.5 py-1 rounded bg-primary/90 text-secondary">★</span>
                      )}
                    </div>
                    {project.videos && project.videos.length > 0 && (
                      <div className="absolute top-3 right-3">
                        <span className="bg-black/60 text-white text-[9px] font-bold px-1.5 py-1 rounded-full flex items-center gap-0.5">
                          <Play size={8} className="fill-white" /> {project.videos.length}
                        </span>
                      </div>
                    )}
                  </div>
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto">
                      <span className="text-primary font-bold text-xs inline-flex items-center group-hover:underline">
                ...
                <div className="p-6 md:p-10">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <div className="lg:col-span-2">
                      ...
                      <div className="flex gap-2 mb-6">
                        <button
                          ...
                          className={`px-5 py-2 rounded-full text-xs font-bold tracking-wider transition-all ${activeTab === "images" ? "bg-secondary text-white shadow" : "bg-muted text-secondary hover:bg-primary/20"}`}
                        >
                        ...
                        {selectedProject.videos && selectedProject.videos.length > 0 && (
                          <button
                            ...
                            className={`px-5 py-2 rounded-full text-xs font-bold tracking-wider transition-all ${activeTab === "videos" ? "bg-secondary text-white shadow" : "bg-muted text-secondary hover:bg-primary/20"}`}
                          >
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {selectedProject.images.map((img, idx) => (
                              <div
                                key={idx}
                                onClick={() => setGalleryImage(img.url)}
                                className="relative h-48 rounded-xl overflow-hidden cursor-pointer group border border-border/50"
                              >
                                ...
                                <div className="absolute inset-0 bg-secondary/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                  <span className="bg-primary text-secondary text-sm font-bold px-4 py-2 rounded-full transform translate-y-4 group-hover:translate-y-0 transition-transform">
                                    View {img.label}
                                  </span>
                                </div>
                    ...
                    <div className="lg:col-span-1">
                      <div className="bg-muted p-6 rounded-xl sticky top-6 border border-border/50">
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <Button asChild variant="outline" className="w-full border-primary text-secondary hover:bg-primary hover:text-secondary font-bold h-12">
                            <a href="tel:+917204767373">📞 Call Now</a>
                          </Button>
                        </div>
                      </div>
                    </div>
          ...
          <div className="fixed inset-0 z-[110] bg-black/95 flex items-center justify-center p-4" onClick={() => setGalleryImage(null)}>
            ...
            <div className="relative inline-flex" onClick={(e) => e.stopPropagation()}>
              <motion.img
                ...
                className="max-w-full max-h-[90vh] object-contain rounded-sm block"
              />
